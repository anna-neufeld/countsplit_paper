...
one_trial_count_split <- function(n,p,filename, k=1,propImp=0.1, sig_strength=5, propLowMedHigh = c(1/2,1/2), eps=c(0.5), c=1) {
  ...
  for (ep in eps) {
    ...
    write(t(res), file=filename, append=TRUE, ncol=15)
  }
...
countsplit <- function(X,Lambda, ep,c=1, gammas=rep(1,NROW(X))) {
  ...
  models <- apply(Xtest, 2, function(u) glm(u~pseudotime, offset=log(gammas), family="poisson"))
  
  # Could it possibly matter here whether or not I put a 1-eps in the offset??? 
  pvals_pseudotime <- sapply(models,function(u) summary(u)$coefficients[2,4])
  CIs.MASS <- sapply(models, function(u) confint(u)[2,])
  CIs.default <- sapply(models, function(u) confint.default(u)[2,])
  CIs.sandwich <- sapply(models, function(u) coefci(u, vcov =sandwich)[2,])
  #CIs.vcovhc3 <- sapply(models, function(u) coefci(u, vcov = vcovHC)[2,])
  
  #mean(apply(CIs.MASS,2,function(u) u[2]-u[1]))
  #mean(apply(CIs.default,2,function(u) u[2]-u[1]))
  ...
  coverage.MASS <- sapply(1:p, function(u) CIs.MASS[1,u] < true_coeffs[u] & CIs.MASS[2,u] > true_coeffs[u])
  coverage.default <- sapply(1:p, function(u) CIs.default[1,u] < true_coeffs[u] & CIs.default[2,u] > true_coeffs[u])
  coverage.sandwich <- sapply(1:p, function(u) CIs.sandwich[1,u] < true_coeffs[u] & CIs.sandwich[2,u] > true_coeffs[u])
  
  trueprincomp <- svd(apply(log(Lambda),2,function(u) u-mean(u)))$u[,1]

  true_cor <- cor(pseudotime, trueprincomp)
  
  return(cbind(pvals_pseudotime,coverage.MASS, coverage.default, coverage.sandwich, true_coeffs, true_cor))
}
...
countsplit_cluster <- function(X,Lambda, ep,c=1, gammas=rep(1,NROW(X)), trueclusters=rep(1,NROW(X))) {
  ...
  models <- apply(Xtest, 2, function(u) glm(u~clusters, offset=log(gammas), family="poisson"))
